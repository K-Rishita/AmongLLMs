...
@pytest.fixture
def mock_crewmate_security():
    """A crewmate located at Security (needed for ViewMonitor)."""
    player = Crewmate(
        name="Player 1",
        color="white",
        personality=None,
    ...
    player.location_info = "You are in Security."
    return player


@pytest.fixture
def mock_env_with_agents():
    """An environment with agents and camera records for ViewMonitor tests."""
    env = Mock()

    agent_a = Crewmate(name="Player 2", color="blue", personality=None, location="Weapons")
    agent_b = Crewmate(name="Player 3", color="green", personality=None, location="Weapons")
    agent_c = Impostor(name="Player 4", color="red", personality=None, location="Navigation")

    env.players = [agent_a, agent_b, agent_c]

    # camera_record keys must match agent.name (which includes color)
    env.camera_record = {
        agent_a.name: "COMPLETE TASK - Fix Wiring",
        agent_b.name: "MOVE from Cafeteria to Weapons",
        agent_c.name: "MOVE from Weapons to Navigation",
    }
...
# ============================================================================
# Test: ViewMonitor Action
# ============================================================================


class TestViewMonitor:
    """Tests for the ViewMonitor action."""

    def test_view_monitor_repr(self):
        """Test that ViewMonitor has the correct string representation."""
        action = ViewMonitor("Security")
        assert repr(action) == "VIEW MONITOR"

    def test_view_monitor_name(self):
        """Test that ViewMonitor has the correct name."""
        action = ViewMonitor("Security")
        assert action.name == "ViewMonitor"

    # ---- can_execute_actions ----

    def test_can_execute_at_security(self, mock_crewmate_security):
        """Test that ViewMonitor is available when player is at Security."""
        env = Mock()
        actions = ViewMonitor.can_execute_actions(env, mock_crewmate_security)
        assert len(actions) == 1
        assert isinstance(actions[0], ViewMonitor)

    def test_cannot_execute_outside_security(self, mock_crewmate):
        """Test that ViewMonitor is NOT available when player is not at Security."""
        env = Mock()
        actions = ViewMonitor.can_execute_actions(env, mock_crewmate)
        assert actions == []

    # ---- execute: empty room ----

    def test_monitor_empty_room(self, mock_crewmate_security, mock_env_with_agents):
        """Test that monitoring an empty room shows 'No one here'."""
        mock_env_with_agents.check_monitor.return_value = []

        action = ViewMonitor("Security")
        action.execute(mock_env_with_agents, mock_crewmate_security, "Reactor")

        assert len(mock_crewmate_security.observation_history) == 1
        record = mock_crewmate_security.observation_history[0]
        assert "Reactor" in record
        assert "No one here" in record

    # ---- execute: agent present in room (non-moving) ----

    def test_monitor_shows_agent_in_room(self, mock_crewmate_security, mock_env_with_agents):
        """Test that monitoring a room shows agents present doing tasks."""
        mock_env_with_agents.check_monitor.return_value = [mock_env_with_agents.players[0]]

        action = ViewMonitor("Security")
        action.execute(mock_env_with_agents, mock_crewmate_security, "Weapons")

        assert len(mock_crewmate_security.observation_history) == 1
        record = mock_crewmate_security.observation_history[0]
        assert "Weapons" in record
        assert "Player 2" in record
        assert "COMPLETE TASK" in record

    # ---- execute: agent entering the room ----

    def test_monitor_shows_enter_for_arriving_agent(self, mock_crewmate_security, mock_env_with_agents):
        """Test that an agent moving INTO the monitored room shows 'enter <room>'."""
        mock_env_with_agents.check_monitor.return_value = [mock_env_with_agents.players[1]]

        action = ViewMonitor("Security")
        action.execute(mock_env_with_agents, mock_crewmate_security, "Weapons")

        record = mock_crewmate_security.observation_history[0]
        assert "Player 3" in record
        assert "enter Weapons" in record

    # ---- execute: agent leaving the room ---


    # ---- execute: unrelated agents not shown ----

    def test_monitor_hides_unrelated_agents(self, mock_crewmate_security, mock_env_with_agents):
        """Test that agents not entering or leaving the monitored room are hidden."""
        mock_env_with_agents.check_monitor.return_value = []
        ...
        record = mock_crewmate_security.observation_history[0]
        assert "No one here" in record
        assert "Player 2" not in record
        assert "Player 3" not in record
        assert "Player 4" not in record

    # ---- execute: multiple agents in same room ----

    def test_monitor_multiple_agents(self, mock_crewmate_security, mock_env_with_agents):
        """Test monitoring a room with multiple agents present."""
        mock_env_with_agents.check_monitor.return_value = [
            mock_env_with_agents.players[0],
            mock_env_with_agents.players[1],
        ]
    ...
    # ---- execute: record format ----

    def test_monitor_record_format(self, mock_crewmate_security, mock_env_with_agents):
        """Test that the monitor record follows the expected format."""
        mock_env_with_agents.check_monitor.return_value = []
        ...
        record = mock_crewmate_security.observation_history[0]
        assert record.startswith("Monitor Record: {")
        assert "Location: Reactor" in record
        assert "Observation: {" in record
        assert record.endswith("}}")

    # ---- execute: observation history grows ----

    def test_monitor_appends_to_observation_history(self, mock_crewmate_security, mock_env_with_agents):
        """Test that each monitor call appends a new entry to observation_history."""
        mock_env_with_agents.check_monitor.return_value = []

        action = ViewMonitor("Security")
        action.execute(mock_env_with_agents, mock_crewmate_security, "Reactor")
        action.execute(mock_env_with_agents, mock_crewmate_security, "Medbay")

        assert len(mock_crewmate_security.observation_history) == 2
        assert "Reactor" in mock_crewmate_security.observation_history[0]
        assert "Medbay" in mock_crewmate_security.observation_history[1]
# ============================================================================
# Run Tests
# ============================================================================
