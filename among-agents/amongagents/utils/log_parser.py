...
class GameLogParser:
    ...
    def extract_voting_history(self) -> List[Dict[str, Any]]:
        ...
        for log_entry in self.agent_logs:
            ...
            if isinstance(response, dict):
                ...
                if not action and "Thinking Process" in response:
                    ...
                    action = thinking_process.get("action", "") if isinstance(thinking_process, dict) else ""
        ...
        for timestep in sorted(votes_by_step.keys()):
            ...
            if vote_tally:
                ...
                players_with_max = [player for player, count in vote_tally.items() if count == max_votes]
    ...
    def extract_game_outcome(self) -> Dict[str, Any]:
        ...
        surviving_players = sorted([p for p in all_players.keys() if p not in eliminated_set])
        
        # Count final impostor/crewmate numbers among survivors
        final_impostor_count = sum(1 for p in surviving_players if all_players.get(p, "").lower() == "impostor")
        final_crewmate_count = len(surviving_players) - final_impostor_count
        
        # Validate game outcome consistency
        self._validate_game_outcome(winner_reason, final_impostor_count, final_crewmate_count, 
                                     surviving_players, eliminated_players, all_players)
    ...
    def _validate_game_outcome(self, winner_reason: str, final_impostor_count: int, 
                                final_crewmate_count: int, surviving_players: List[str],
                                eliminated_players: List[str], all_players: Dict[str, str]):
        """Validate that extracted game outcome matches the stated winner reason."""
        warnings = []
        
        # Check if outcome matches the winner reason
        if "impostor" in winner_reason.lower() and "eliminated" in winner_reason.lower():
            # Should have 0 impostors surviving
            if final_impostor_count > 0:
                impostor_survivors = [p for p in surviving_players if all_players.get(p, "").lower() == "impostor"]
                warnings.append(f"Game claims 'Impostors eliminated' but {final_impostor_count} impostor(s) survived: {impostor_survivors}")
        
        elif "impostor" in winner_reason.lower() and "win" in winner_reason.lower():
            # Impostors won - should have impostors alive
            if final_impostor_count == 0:
                warnings.append(f"Game claims 'Impostors win' but no impostors survived")
            ...
            warnings.append(f"Game claims 'Tasks completed' but task completion tracking not implemented in parser")
        ...
        if total_players != expected_players:
            warnings.append(f"Player count mismatch: {total_players} accounted for vs {expected_players} expected")
        ...
        if warnings:
            ...
            print(f"   This may indicate incomplete/malformed LLM outputs in the original game logs.\n", file=sys.stderr)
...
def update_existing_game_log(file_path: str, output_path: Optional[str] = None) -> Dict[str, Any]:
