...
class MapUI:
    ...
    def reset(self):
        ...
        self.canvas = tk.Canvas(
            self.master, width=self.map_image.width(), height=self.map_image.height()
        )
        self.canvas.pack()
        ...
        self.task_progress = tk.Canvas(
            self.master, width=self.canvas.winfo_width(), height=30
        )
        ...
        self.activity_log = ScrollingActivityLog(
            self.master, width=self.map_image.width()
        )
    ...
    def draw_map(self, env):
        ...
        for room, roominfo in self.room_coords.items():
            ...
            # Start drawing players from here
            start_coords = (min_x + space, part_y)
            temp_coords = start_coords
            for player in players:
                self.canvas.create_oval(
                    temp_coords[0],
                    temp_coords[1],
                    temp_coords[0] + player_size,
                    temp_coords[1] + player_size,
                    fill=player.color,
                )
                ...
                if temp_coords[0] > max_x - player_size - space:
                    temp_coords = (
                        start_coords[0],
                        temp_coords[1] + player_size + space,
                    )

        self.draw_task_progress(
            env.task_assignment.check_task_completion(), game_over=env.check_game_over()
        )
    ...
    def draw_task_progress(self, task_progress, game_over):
        ...
        self.task_progress.create_rectangle(
        ...
        if game_over != 1:
            self.task_progress.create_rectangle(
                x1,
                y1,
                x1 + (x2 - x1) * task_progress,
                y2,
                fill="green",
                outline="black",
            )
            self.progress_label.config(
                text=f"Task Progress: {task_progress * 100:.1f}%"
            )
    ...
    def report(self, text):
        ...
        self.canvas.create_text(
            (width / 2, height / 2), text=text, fill="black", font=("Calibri", 10)
        )
        self.master.update()
...
class ScrollingActivityLog(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        scrollbar = tk.Scrollbar(
            self, orient="vertical", command=self.activity_log.yview
        )
