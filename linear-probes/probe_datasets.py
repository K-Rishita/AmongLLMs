...
def phi4_format(system, user, assistant):
    ...
    assistant_part = (
        f"<|im_start|>assistant<|im_sep|>{assistant}"
        if assistant
        else "<|im_start|>assistant<|im_sep|>"
    )
...
def llama3_format(system, user, assistant):
    ...
    user_part = (
        f"<|start_header_id|>user<|end_header_id|>{user}<|eot_id|>" if user else ""
    )
    assistant_part = (
...
class ActivationDataset(Dataset):
    ...
    def get_train(
        ...
        train_size = min(
            int(len(chunk_data) * (1 - self.test_split)), len(chunk_data) - 1
        )
        ...
        for attempt in range(max_attempts):
            ...
            ):  # If we've tried max_attempts times and still have no samples
                return train_loader, None
    ...
    def get_train_data_stats(self, chunk_idx: int = 0) -> dict:
        ...
        return {"total_samples": train_size, "class_distribution": class_dist}
...
class TruthfulQADataset(ActivationDataset):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            tokenizer,
            device,
            config["activation_size"],
        )
        self.data_path: str = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "./data/TruthfulQA/TruthfulQA.csv",
        )
        self.tqa_df = pd.read_csv(self.data_path)
        self.activations_dir: str = (
            f"data/{self.name}_{config['short_name']}_acts_{config['layer']}/"
        )
    ...
    def process_row(self, row, num_tokens: int = 5, seq_len: int = 1024):
        ...
        correct_inputs = self.tokenizer(
            correct_prompt, return_tensors="pt", max_length=seq_len, truncation=True
        ).to(self.device)
        incorrect_inputs = self.tokenizer(
            incorrect_prompt, return_tensors="pt", max_length=seq_len, truncation=True
        ).to(self.device)
        chunk_data = []
        with t.no_grad():
            ...
            correct_acts = (
                [correct_activations[i] for i in range(-num_tokens, 0)]
                if num_tokens
                else [correct_activations[i] for i in range(len(correct_activations))]
            )
            chunk_data.append((correct_acts, 1))

            self.activation_cache.clear_activations()
            self.model.forward(
                incorrect_inputs["input_ids"],
                attention_mask=incorrect_inputs["attention_mask"],
                pad_token_id=self.tokenizer.eos_token_id,
            )
            incorrect_activations = self.activation_cache.activations[0][0]
            incorrect_acts = (
                [incorrect_activations[i] for i in range(-num_tokens, 0)]
                if num_tokens
                else [
                    incorrect_activations[i] for i in range(len(incorrect_activations))
                ]
            )
...
class DishonestQADataset(ActivationDataset):
    ...
    def row_to_prompts(self, row: pd.Series) -> str:
        ...
        incorrect_q = self.format(
            "Pretend you're a dishonest assistant.", question, None
        )
    ...
    def process_row(self, row, num_tokens: int = 5, seq_len: int = 1024):
        ...
        with t.no_grad():
            ...
            full_output_inputs = self.tokenizer(
                full_output_str,
                return_tensors="pt",
                max_length=seq_len,
                truncation=True,
            ...
            self.model.forward(
                full_output_inputs["input_ids"],
                attention_mask=full_output_inputs["attention_mask"],
                pad_token_id=self.tokenizer.eos_token_id,
            )
            correct_activations = self.activation_cache.activations[0][0]
            correct_acts = (
...
class AmongUsDataset(ActivationDataset):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            test_split, "AmongUs", model, tokenizer, device, config["activation_size"]
        )
        self.name: str = "AmongUs"
        base_dir = os.path.dirname(os.path.abspath(__file__))
        self.agent_logs_path: str = os.path.join(
            base_dir, raw_path, expt_name + "/agent-logs-compact.json"
        )
    ...
    def agent_logs_row_to_full_prompt(self, row: pd.Series) -> str:
        ...
        formatted_prompt = self.format(
            system_prompt if system_prompt else None, user_prompt, assistant_prompt
        )
    ...
    def process_row(self, row, num_tokens, seq_len):
        ...
        inputs = self.tokenizer(
            phi_format_prompt, return_tensors="pt", max_length=seq_len, truncation=True
        ).to(self.device)
        with t.no_grad():
            self.activation_cache.clear_activations()
            self.model.forward(
            ...
            if not self.activation_cache.activations:
                raise ValueError(
                    "No activations found. Ensure the model and activation cache are set up correctly."
                )
            activations = self.activation_cache.activations[0][0]
            if num_tokens:
                if len(activations) < num_tokens:
                    raise ValueError(
                        f"Not enough activations to extract {num_tokens} tokens. Available: {len(activations)}"
                    )
    ...
    def populate_dataset(
        ...
        print(
            f"Finished processing {rows_to_cache} rows into {self.num_total_chunks} chunks"
        )
...
class RolePlayingDataset(ActivationDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.data_path: str = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "./data/Roleplaying/phi4_rollouts.csv",
        )
    ...
    def process_row(self, row, num_tokens: int = 5, seq_len: int = 1024):
        ...
        inputs = self.tokenizer(
            prompt, return_tensors="pt", max_length=seq_len, truncation=True
        ).to(self.device)
        chunk_data = []
        with t.no_grad():
            self.activation_cache.clear_activations()
            ...
            activations = self.activation_cache.activations[0][0]
            acts = (
                [activations[i] for i in range(-num_tokens, 0)]
                if num_tokens
...
class RepEngDataset(ActivationDataset):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            test_split, "RepEng", model, tokenizer, device, config["activation_size"]
        )
        self.data_path: str = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
    ...
    def row_to_prompts(self, row: pd.Series) -> str:
        ...
        user: str = (
            f"Pretend you're a {label} person making statements about the world."
        )
...
class ApolloProbeDataset(ActivationDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.data_path: str = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "./data/ApolloProbe/common_claim_true_false.csv",
        )
        self.df = pd.read_csv(self.data_path)
        self.activations_dir: str = (
            f"data/{self.name}_{config['short_name']}_acts_{config['layer']}/"
    ...
    def process_row(self, row, num_tokens: int = 5, seq_len: int = 1024):
        ...
        correct_assistant_start = correct_prompt.find(
            "<|im_start|>assistant<|im_sep|>"
        ) + len("<|im_start|>assistant<|im_sep|>")
        incorrect_assistant_start = incorrect_prompt.find(
            "<|im_start|>assistant<|im_sep|>"
        ) + len("<|im_start|>assistant<|im_sep|>")

        # Get token indices for the user message start positions
        correct_assistant_token_start = len(
            self.tokenizer.encode(
                correct_prompt[:correct_assistant_start], add_special_tokens=False
            )
        )
        incorrect_assistant_token_start = len(
            self.tokenizer.encode(
                incorrect_prompt[:incorrect_assistant_start], add_special_tokens=False
            )
        )
        ...
        with t.no_grad():
            ...
            correct_activations = self.activation_cache.activations[0][0]
            correct_acts = [
                correct_activations[correct_assistant_token_start + i]
            ...
            incorrect_activations = self.activation_cache.activations[0][0]
            incorrect_acts = [
                incorrect_activations[incorrect_assistant_token_start + i]
                for i in range(num_tokens)
            ]
            chunk_data.append((incorrect_acts, 0))
        return chunk_data
