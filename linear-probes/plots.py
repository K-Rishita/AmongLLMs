...
def plot_roc_curve_eval(labels, probe_outputs, labels_2=None, names=None):
    ...
    if labels_2 is not None:
        ...
        curve_name_2 = (
            f"ROC (AUC = {roc_auc_2:.3f})"
            if names is None or len(names) < 2
            else f"{names[1]} (AUC = {roc_auc_2:.3f})"
        )
...
def plot_roc_curves(
    probe_eval, title="ROC Curves for Different Behaviors (all actions)", behaviors=None
):
    """
    ...
    # Update layout
    fig.update_layout(height=350, width=900, title_text=title, showlegend=True)
...
def plot_metrics(
    ...
    for label, y_values in metrics.items():
        fig.add_trace(
            go.Scatter(
                x=list(range(len(y_values))), y=y_values, name=label, line=dict(width=2)
            )
        )
