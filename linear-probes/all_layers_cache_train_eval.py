...
def cache_dataset_layer_acts(dataset_name: str, layer):
    ...
    if dataset_name == "AmongUsDataset":
        ...
        dataset = eval(dataset_name)(
            config, model=model, tokenizer=tokenizer, device=device, test_split=1.0
        )

    eval(f"model.{config['hook_component']}").register_forward_hook(
        dataset.activation_cache.hook_fn
    )
    num_tokens = 5
...
for layer in layers_to_work_on:
    ...
    for dataset_name in datasets_to_train:
        ...
        print(
            f"Training probe on {len(train_loader)} batches and {len(train_loader.dataset)} samples."
        )
        probe.fit(train_loader, epochs=config["probe_training_epochs"])

        checkpoint_path = (
            f"checkpoints/{dataset_name}_probe_{config['short_name']}_layer{layer}.pkl"
        )
...
def evaluate_probe(
    ...
    if plot_stuff:
        ...
        fig.write_image(
            f"results/{dataset_name}_{config['short_name']}/layer_{layer}_roc_TQA.pdf",
            scale=1,
        )
    ...
    if plot_stuff:
        fig, roc = plot_roc_curve_eval(labels, av_probe_outputs)
        fig.write_image(
            f"results/{dataset_name}_{config['short_name']}/layer_{layer}_roc_DQA.pdf",
            scale=1,
        )
    ...
    if plot_stuff:
        fig, roc = plot_roc_curve_eval(labels, av_probe_outputs)
        fig.write_image(
            f"results/{dataset_name}_{config['short_name']}/layer_{layer}_roc_RepEng.pdf",
            scale=1,
        )
    ...
    fpr_lying, tpr_lying, _ = roc_curve(
        probe_eval["lying_truth"], probe_eval["probe_output"]
    )
    roc_auc_lying = auc(fpr_lying, tpr_lying)

    fpr_deception, tpr_deception, _ = roc_curve(
        probe_eval["deception_truth"], probe_eval["probe_output"]
    )
    ...
    rocs["AmongUs"] = [
        {
            "name": "lying",
            "fpr": fpr_lying.tolist(),
            "tpr": tpr_lying.tolist(),
            "auc": roc_auc_lying,
        },
        {
            "name": "deception",
            "fpr": fpr_deception.tolist(),
            "tpr": tpr_deception.tolist(),
            "auc": roc_auc_deception,
        },
    ]
    ...
    if plot_stuff:
        ...
        fig.write_image(
            f"results/{dataset_name}_{config['short_name']}/layer_{layer}_roc_AmongUs.pdf",
            scale=1,
        )
...
for probe_dataset_name in datasets_to_eval:
    ...
    for layer in layers_to_work_on:
        ...
        rocs = evaluate_probe(
            ...
            probe,
            config,
            plot_stuff=True if (layer == 20) else False,
        )
...
if len(layers_to_work_on) == base_config["num_layers"] or not os.path.exists(
