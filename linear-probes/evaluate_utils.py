...
def evaluate_probe_on_string(
    prompt, model, tokenizer, probe, dataset, device, tokens_to_average=5
):
    """
    ...
    with t.no_grad():
        ...
        probe_outputs = [
            round(probe.evaluate_single_activation(t.tensor(act, device=device)), 4)
            for act in last_n_activations
        ]
...
def evaluate_probe_on_dataset(
    test_df, model, tokenizer, probe, dataset, device, tokens_to_average=5, verbose=True
):
    """
...
def evaluate_probe_on_activation_dataset(
    chunk_data, probe, device, num_tokens=None, verbose=True, output_token_scores=False
):
    """
